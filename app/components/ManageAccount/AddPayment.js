  ...
  useEffect(() => {
    ...
    // if user has already used trial, set up payment intent to be charged right away
      ...
      const getPaymentIntent = async () => {
        const response = await axios.post('/api/retrieve-payment-intent', {customerId: user.stripeCustomerId});
...
const Payment = ({user, price}) => {
  ...
  <form className='w-full'>
    <p className="text-gray-700 font-bold text-2xl border-b border-gray-300 pb-3 mb-4 mt-12">
      ...
      {!user.isCanceled && <p className="mb-6 text-sm text-gray-500">You will not be charged until your 7 day free trial period is over.</p>}
      {user.isCanceled && <p className="mb-6 text-sm text-gray-500">You are now signing up for the {price === 'monthly' ? 'monthly' : 'annual'} plan for {price === 'monthly' ? '$19.99/mo' : '$199.99/yr'}</p>}
      <PaymentElement/>
      <button disabled={!stripe || isSubmitting ? true : false} onClick={handleSubmit} className={`h-[40px] mt-8 w-[100px] flex justify-center items-center text-sm text-white font-medium rounded-md transition-all duration-300 mr-2 ${
        ...
        {isSubmitting ? <ButtonSpinner /> : user.isCanceled ? `Pay ${price === 'monthly' ? '$19.99' : '$199.99'}` : 'Submit' }
      </button>
      {errorMessage && (
